...
class Processor:
    ...
    def __init__(self, args):
        ...
        # dummy_input = torch.randn(1, 3, 300, 25,2).to(next(self.model.parameters()).device)  
        # flops = FlopCountAnalysis(self.model, dummy_input)
        # print(f"GFLOPs value of the model: {flops.total() / 1e9:.2f} GFLOPs")      
        #############################################################################
        ...
        # DWA için başlangıç değerleri
        self.num_losses = 3  # DWA için kullanılacak kayıp fonksiyonu sayısı (output_6, output_11, output_25)
        self.loss_weights = [1.0] * self.num_losses
        self.previous_losses = [0.0] * self.num_losses
        self.T = 2  # Sıcaklık parametresi
        self.start()
    ...
    def start(self):
        ...
        for epoch in range(1, self.args.num_epoch+1):
            ...
            if epoch % 10 == 0:
                path = self.args.work_dir + f'/epoch_{epoch}.pt'
                torch.save(self.model.state_dict(), path)    
...
# TRAIN 
    ...
    def train(self, epoch):
        ...
        # Her bir kayıp fonksiyonu için toplam kayıpları tutmak
        ...
        for i, (data, label, _) in enumerate(process):
            with torch.set_grad_enabled(True):
                ...
                # Her bir kayıp fonksiyonunu hesapla
        ...
        if epoch >= 2:
            ...
            # Önceki kayıpları güncelle
            self.previous_losses = avg_losses.copy()
        else:
            # İlk epoch için önceki kayıpları ayarla
            self.previous_losses = avg_losses.copy()
            # Ağırlıkları eşit tut
            self.loss_weights = [1.0] * self.num_losses
