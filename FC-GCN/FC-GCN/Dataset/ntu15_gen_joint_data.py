...
# NTU15: seçilen action'lar (1-based NTU action id)
KEEP_ACTIONS = [6, 7, 14, 20, 22, 23, 24, 26, 27, 35, 36, 38, 40, 43, 49]

# 0–14 label map (MODEL İÇİN DOĞRUSU)
A2NEW = {a: i for i, a in enumerate(KEEP_ACTIONS)}
# 6->0, 7->1, 14->2, ..., 49->14
...
def gendata(data_path, out_path, ignored_sample_path=None, benchmark='xview', part='eval'):
    ...
    for filename in os.listdir(data_path): #sorted ifadesi yoktu eklendi
        ...
        if issample:
            # SADECE NTU15 sınıfları al
            if action_class in A2NEW:
                sample_name.append(filename)
                sample_label.append(A2NEW[action_class])  # 0–14
    ...
    for i, s in enumerate(tqdm(sample_name)):
        ...
        T = 64
        valid_frame = min(T, data.shape[1])
        fp[i, :, :valid_frame, :, :] = data[:, :valid_frame, :, :]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='Ntu15_Skeleton/')
    parser.add_argument('--ignored_sample_path',
                        default='Missing_Data/NTU_RGBD_samples_with_missing_skeletons.txt')
    parser.add_argument('--out_folder', default='Ntu15/')
